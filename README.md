...
Run the small inference script converted from the notebook. The script mirrors the notebook flow (model loading, optional pron_inpaint patching, and generation) and saves generated speech as WAV files in `--outdir`.
...
python infer.py \
  ...
  --outdir outputs \
  --device cuda \
  --qwen_pretrain_model /notebooks/projects/tts2025/results/cosyvice2-20250907/blank \
  --cosyvoice_dir /notebooks/projects/CosyVoice/pretrained_models/CosyVoice2-0.5B \
  --inpaint_weights /notebooks/projects/CosyVoice-Inpaint/results/inpaint/pytorch_model.bin
```

Notes:

- `--prompt_audio` should be a short reference WAV used for zero-shot voice prompting.
- `--inpaint_weights` is optional; if omitted the script will run without the inpaint fine-tuned weights.
- Output WAVs are written to `--outdir` (default `outputs`).

---

## Samples 🎧

The repository contains example generated audio in the `assets/` directory. Click play on the embedded players below to listen to the generated samples.

**Generated samples:**

- **上[soeng5]堂終於講到分[fan1]數**

<audio controls>
  <source src="assets/上[soeng5]堂終於講到分[fan1]數.mp3" type="audio/mpeg">
  Your browser does not support the audio element.
</audio>

- **上[soeng6]堂終於講到分[fan3]數**

<audio controls>
  <source src="assets/上[soeng6]堂終於講到分[fan3]數.mp3" type="audio/mpeg">
  Your browser does not support the audio element.
</audio>
...
<audio controls>
  <source src="assets/我識得講個撚[nan2]字.wav" type="audio/wav">
  Your browser does not support the audio element.
</audio>

> Files are stored in `assets/`. These were generated by running `infer.py` and exported as MP3 or WAV files.
